...
Capacity is a production blocker until a protocol-level solution exists. Claims require a new note leaf; once the note tree is full, outstanding allocations cannot be claimed in this immutable pool. Deposits and distribution creation also require note leaves, including zero-value change. Withdrawals of existing notes do not insert leaves and remain possible under their normal proof requirements, but do not release capacity. A client-side capacity check cannot reserve space for later claims. Separately, permanently lost recipient keys have no sender reclaim or administrator recovery path in v1.
