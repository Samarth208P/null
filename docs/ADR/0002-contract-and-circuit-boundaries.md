...
There is no public withdrawal or private-note transfer circuit in v1. The intended MVP endpoint is a recipient's private note. Only valueless testnet assets are appropriate. The README/demo must say that source is implemented, that tests were deferred, and whether actual proof/deployment evidence exists; it must not claim an audited or complete live privacy system from a local UI session.
