...
| [ENS explorer](https://explorer.ens.dev/) | Search the actual recipient name, such as `inbox.nullpay2026.eth`. Show `null.paymentProfile` only after publication. Use the app's permission comparison for payment-record versus website-record access. |
