...
The SDK is now available as the npm developer preview `@samarth208p/null-payouts@0.1.0-preview.2`; internal workspace packages retain their original names. Publishing the SDK did not deploy v0.3. The reference UI's v0.3 balance field is capability-gated and cannot activate partial exits on the old pool. A gas sponsor pays public transaction costs; it does not make deposits/withdrawals anonymous or imply free operation for the sponsoring business.
