...
The workspace preference is saved in local storage, under a versioned key scoped to the authenticated Privy ID. Invalid preferences return the user to onboarding; unavailable storage permits the current session and displays an explanation. After exporting or restoring a Payment ID backup, this tab keeps an AES-GCM encrypted session containing the same receiving keys and backup state. Its non-exportable browser key is stored separately in IndexedDB; no backup password or plaintext receiving keys are saved. Refresh restores this session before checking the current ENS ownership and payment record, then opens the linked inbox without repeating setup. Drafts and balances still reset on reload. Changing workspace type preserves the active identity. Explicit sign-out removes the session ciphertext and browser key.

An existing name without a usable browser session offers backup restoration instead of repeating the three setup steps. Restoring an older inbox once enables continuity on subsequent refreshes in that tab. A new browser session or device still needs the password-protected backup, and unavailable session storage displays a warning. The session cache is local refresh convenience, not protection against compromised same-origin JavaScript or a replacement for the backup file. ENS lookup failures offer a retry and never authorize entry using a saved name alone. Encrypted recovery is available in the inbox and Settings; the sign-out confirmation explains the loss of unsaved state. Existing organization membership, wallet ownership, treasury policy, and server authorization checks remain authoritative. Choosing an organization workspace never authorizes a transaction.
