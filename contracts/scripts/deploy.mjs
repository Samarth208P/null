...
const journalPath = resolve(root, `.artifacts/deployment-${chainId}.json`);
const fingerprint = sha(JSON.stringify({ chainId, asset, deployer: account.address, integrity, artifacts }));
const journal = existsSync(journalPath) ? readJson(`.artifacts/deployment-${chainId}.json`) :
...
const steps = [
  ...
  { id: 'nullPool', ...compiled.NullPool, args: a => [asset, a.poseidon3, a.nullAuthRegistry, a.shieldVerifier, a.createDistributionVerifier, a.claimVerifier] },
];
...
const plan = { mode: 'deployment-plan', chainId, asset, deployer: account.address,
  ...
  libraryAliases, warning: 'Unaudited testnet prototype; no withdrawals. Contract deployment does not validate the proof flow.' };
...
manifest.contracts = Object.fromEntries(['nullPool', 'nullAuthRegistry', 'poseidon3', 'shieldVerifier', 'createDistributionVerifier', 'claimVerifier'].map(name => [name, addresses[name]]));
...
for (const [name, expected] of Object.entries({ ASSET: asset, HASHER: addresses.poseidon3, AUTH_REGISTRY: addresses.nullAuthRegistry,
  shieldVerifier: addresses.shieldVerifier, createDistributionVerifier: addresses.createDistributionVerifier, claimVerifier: addresses.claimVerifier })) {
...
for (const name of ['shieldVerifier', 'createDistributionVerifier', 'claimVerifier']) {
...
if (existsSync(output)) {
  const previous = readJson(`deployments/${chainId}.json`);
...
updateRootEnv({ NULL_POOL_ADDRESS: addresses.nullPool, NULL_MANIFEST_PATH: `deployments/${chainId}.json`,
