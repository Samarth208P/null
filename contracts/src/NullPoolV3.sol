...
contract NullPoolV3 {
    ...
    IVerifier public immutable partialWithdrawVerifier;
    bytes32 public immutable partialWithdrawVerifierCodeHash;
    ...
    constructor(IERC20 asset, IPoseidon3 hasher, NullAuthRegistry registry,
        ...
        IVerifier withdrawProofVerifier, IVerifier partialWithdrawProofVerifier)
    {
        ...
        if (address(partialWithdrawProofVerifier).code.length == 0) revert InvalidDependency();
        partialWithdrawVerifier = partialWithdrawProofVerifier;
        partialWithdrawVerifierCodeHash = address(partialWithdrawProofVerifier).codehash;
        ASSET = asset;
        HASHER = hasher;
    ...
    /// @notice Recipient-only partial exit with a hidden remaining balance.
    /// The original note commitment and amount are private proof witnesses.
    function withdrawPartial(bytes calldata proof, bytes32[] calldata inputs) external nonReentrant {
        _context(inputs, 11);
        _deadline(inputs[10]);
        if (!notes.known(uint256(inputs[3])) || !AUTH_REGISTRY.isKnownAuthRoot(uint256(inputs[4]))) revert RootStale();
        uint256 nullifier = uint256(inputs[5]);
        ...
        uint256 amount = uint256(inputs[7]);
        if (nullifier == 0 || destination == 0 || destination > type(uint160).max ||
            address(uint160(destination)) == address(this) || amount == 0 || amount > type(uint64).max) revert InvalidPublicInputs();
        if (spentNoteNullifier[nullifier]) revert NullifierSpent();
        _verify(partialWithdrawVerifier, partialWithdrawVerifierCodeHash, proof, inputs);
        address recipient = address(uint160(destination));
        uint256 poolBefore = ASSET.balanceOf(address(this));
        uint256 recipientBefore = ASSET.balanceOf(recipient);
        spentNoteNullifier[nullifier] = true;
        _insertNote(uint256(inputs[8]), 1);
        (bool ok, bytes memory result) = address(ASSET).call(abi.encodeCall(IERC20.transfer, (recipient, amount)));
        if (!ok || (result.length != 0 && (result.length != 32 || !abi.decode(result, (bool))))) revert TransferFailed();
        if (ASSET.balanceOf(address(this)) + amount != poolBefore || ASSET.balanceOf(recipient) != recipientBefore + amount) revert TransferFailed();
        emit Withdrawn(nullifier, recipient, uint64(amount));
    }
