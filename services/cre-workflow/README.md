...
Set `payrollBaseUrl` to the complete collection endpoint, for example `https://payroll.example.com/batches`. The handler appends `/<batchId>`; it does not add `/batches`. Custom API collection paths are supported. The production handler requires HTTPS. The separate `cre-starter/payroll` local workflow uses its configured loopback `/batches` endpoint and returns `cre-tee-simulation`; the production handler returns `cre-tee`.
