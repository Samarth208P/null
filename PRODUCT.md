...
For every new live distribution, recipients must link a Sepolia ENSv2 payment name and the sender must confirm its resolved profile. Reject raw IDs and incomplete name coverage; recheck before preparation, approval and submission. Require live ENS verification before opening either private workspace, including direct routes. Recovery and name linking remain available in setup; claims and withdrawals require entry through the linked account. Preserve an already open operation for safe reconciliation, while fresh action checks reject a changed identity. This is an application constraint, not an immutable-contract or circuit invariant. Names and linked public profiles are public.
...
The public entry is a developer quickstart and read-only preparation example; it loads without Privy. The reference app authenticates before displaying private workspace content. On the first authenticated visit, ask for account type. Individuals choose an ENS payment name with the configured namespace suffix visible; full names are also accepted. Saving that preference neither registers a name nor publishes a profile. Individuals back up or restore their original Payment ID and explicitly link/verify the name before opening their inbox. Both individuals and organizations select a Sepolia ENS identity. Organizations also keep an internal workspace label and see overview, payouts and treasury navigation. Wallet labels use current ENS address resolution or verified name ownership; ownership is labeled separately and never implies organization membership or a transfer destination. Organization identity is checked against the signer returned by the authenticated organization service; every payment policy must use that same signer. A name saved in local preferences is never proof of linking. Unverified names, missing names, and unavailable lookups stay explicit. Wallet addresses and policy identifiers appear in expandable technical details. Withdrawals and editor grants accept confirmed ENS address records and recheck them before submission; withdrawals and removal of existing editor access retain a manual-address recovery path. A sending organization does not need to publish a receiving Payment ID.

## Withdrawal versions

The fresh Sepolia v0.3 pool is the sole active reference-app deployment and supports recipient-only partial withdrawals: a proof spends the original note, transfers the chosen public amount and inserts a new commitment for the hidden remainder. Full withdrawals retain the existing path. The client refuses partial withdrawals unless a v0.3 manifest and matching verifier are configured. There is no automatic migration of existing funds between immutable pools.
