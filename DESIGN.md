...
The top-right ENS account menu uses the existing pale overlay, 12px corners, and menu shadow, with a 340px maximum width bounded by the viewport. Its Privy wallet section shows the full wrapping personal funding address with a copy action, a selector when multiple personal wallets are connected, and only Sepolia ETH and USDC balances in tabular numerals. Keep the separate organization approval wallet excluded. Manual refresh and a 30-second refresh while the menu is open and the page is visible update these public wallet funds. Loading, missing-wallet, and unavailable-balance states remain explicit; the shared privacy control masks both amounts. Copy and refresh actions expand to 44px on mobile.
