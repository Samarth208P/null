...
export function DistributionWizard() {
  ...
  return <>
    ...
    <div className="wizard-layout"><div className="wizard-main"><ol className="stepper">{steps.map((label, index) => <li key={label} className={step === index ? 'current' : step > index ? 'complete' : ''} aria-current={step === index ? 'step' : undefined}><span>{step > index ? <Check size={13} /> : index + 1}</span><strong>{label}</strong></li>)}</ol>
      ...
      {step === 1 && store.mode === 'testnet' && <label className="checkbox-field"><input type="checkbox" checked={creRequired} onChange={event => { setCreRequired(event.target.checked); setCreVerified(false); setCrePayroll(undefined); setCompiled(undefined); }} /><span>Verify this payment with Chainlink CRE’s local confidential-workflow simulator before review.</span></label>}
      {step === 2 && compiled && <><div className="form-section-heading"><span className="inline-success"><ShieldCheck size={20} />Payment details checked</span><h2>{creRequired && !creVerified ? "Complete the CRE check" : "Ready to review"}</h2><p>Your recipient details and amounts are ready. Nothing has been sent.</p></div><div className="preflight-summary"><KeyValue label="Recipients">{compiled.realCount}</KeyValue><KeyValue label="Total">{money(total, true)} USDC</KeyValue></div><Notice>{store.mode === 'sandbox' ? 'Next, review and complete your practice payment. It will be available for this browser session only.' : 'Next, review your payment. Your organization must approve it before you can send it on the test network.'}</Notice></>}
      ...
      {step === 2 && compiled && creRequired && creVerified && <Notice>CRE simulation output matches this payment. This verifies the encrypted result, not remote enclave execution.</Notice>}
