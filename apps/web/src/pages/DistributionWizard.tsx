...
import { PaymentNameError, paymentDestination, recheckPaymentNames } from '@null-protocol/ens';
...
export function DistributionWizard() {
  ...
  const setRows = (next: SetStateAction<RecipientRow[]>) => { rowVersion.current++; setRowsState(next); setCompiled(undefined); setCreVerified(false); };
  ...
  async function importCsv(file?: File) {
    ...
    try {
      ...
      if (!headers || headers[0] !== 'name' || headers[1] !== 'amount' || !['privacyprofile', 'paymentdestination'].includes(headers[2]) || headers.length !== 3) throw new Error('Use the CSV template. Put each person’s ENS name or Payment ID in the paymentDestination column.');
  ...
  async function prepare() {
    ...
    try {
      ...
      await recheckPaymentNames(ensClient, rows.flatMap(row => row.paymentName ? [row.paymentName] : []));
