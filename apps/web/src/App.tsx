...
function WorkspaceApp() {
  ...
  return <div className="app-shell">
    ...
    <aside ref={sidebar} className={`sidebar ${mobileOpen ? 'sidebar-open' : ''}`} inert={mobileViewport && !mobileOpen || undefined} aria-label="Main navigation" id="workspace-navigation">
      <div className="sidebar-brand-row"><a className="brand" href={`#/${home}`} aria-label="NULL home"><Logo size={22} animated className="brand-logo" /><span>NULL</span></a><button className="icon-button mobile-menu" onClick={() => { setMobileOpen(false); menuButton.current?.focus(); }} aria-label="Close navigation"><X size={20} /></button></div>
