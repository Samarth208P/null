...
export function PaymentDestination({ row, onChange }: { row: RecipientRow; onChange: (destination: string, profile: string, paymentName?: PaymentNameSnapshot) => void }) {
  ...
  async function check() {
    ...
    try {
      ...
      if (parsed.kind === 'profile') { onChange(parsed.profile, parsed.profile); return; }
  ...
  return <div className="payment-destination">
    <label className="field" htmlFor={fieldId}>ENS name or Payment ID</label>
    ...
    }} placeholder="your-name.eth or st:eth:0x…" /><Button variant="secondary" icon={Search} busy={busy} disabled={!destination.trim()} onClick={() => void check()}>Check</Button></div>
    <p className="field-hint" id={`${fieldId}-hint`}>Sepolia names or NULL Payment IDs only. No wallet addresses.</p>
    ...
    {candidate && <div className={`name-resolution ${changed ? 'name-resolution-changed' : ''}`} role="status">
      ...
      <Button variant="secondary" icon={Check} onClick={() => { onChange(candidate.name, candidate.profile, candidate); store.rememberPaymentName(candidate); setCandidate(undefined); }}>{changed ? 'I confirmed this change' : 'Use this Payment ID'}</Button>
    </div>}
