...
export function LiveBalanceRecoveryLoader({ open, onClose, identityKeys, onRecovered }: { open: boolean; onClose: () => void; identityKeys: ProfileKeys; onRecovered: (value: { treasuryBalance: bigint | null; notes: PrivateNote[] }) => void }) {
  if (!open) return null;
  return <Suspense fallback={<Modal open title="Opening encrypted recovery" onClose={onClose}><p className="modal-copy" role="status">Loading local recovery and chain verification tools…</p></Modal>}><LiveBalanceRecovery open onClose={onClose} identityKeys={identityKeys} onRecovered={onRecovered} /></Suspense>;
}
